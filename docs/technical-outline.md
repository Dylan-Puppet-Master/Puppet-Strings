...
- **Offered clinics are staffed**: a standing `CLINIC` request, `REQUEST EACH activities.clinics.offerings`, resolves each offering `(a, b)` of the target date to `REQUEST activities.clinics.a DURING blocks.b`, one copy per offering.
...
- **Offerings are names, not requests** (Puppet Master, 2026-09-25). Every load made one
  ...
  filled the request manager with requests nobody changed. The Offerings tab is now names,
  under `activities.clinics.offerings` beside the clinics themselves, as the cabin act board
  is under `activities.cabin_acts`: `offerings.clinic_2.archery_1_2` and
  `offerings.clinic_2`. An offering asked for with no subject is its clinic in its block:
  one standing `REQUEST EACH activities.clinics.offerings` does what the generated requests
  did, and `generate.py` is gone.
